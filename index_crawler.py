...
class ExportForm:
    def __init__(self, conn, scrptdir):
        ...
        self.btnSelectAll_idx = ttk.Button(master, width=10, text='Select All', style="TButton",
                                           command=self.selectall_idx)
        self.btnSelectAll_idx.grid(row=3, column=0, sticky=tkinter.W)

        self.btnSelectAll_entry = ttk.Button(master, width=10, text='Select All', style="TButton",
                                             command=self.selectall_ent)
        self.btnSelectAll_entry.grid(row=3, column=2, sticky=tkinter.W)

        self.btnClearAll_idx = ttk.Button(master, width=10, text='Clear All', style="TButton",
                                          command=self.clearall_idx)
        self.btnClearAll_idx.grid(row=3, column=0, sticky=tkinter.E)

        self.btnClearAll_entry = ttk.Button(master, width=10, text='Clear All', style="TButton",
        ...
        def onselect_Entry(evt):
            ...
            for row in result:
                ...
                for p in pages:
                    count += 1
                    self.lstPages.insert(tkinter.END, ' | '.join((row[0], p)))
            # if count == 1:
